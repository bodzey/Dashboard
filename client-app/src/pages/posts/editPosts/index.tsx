...
const EditPost: React.FC = () => {
  ...
  const { selectedPost } = useTypedSelector((store) => store.PostReducer);
  const store = useTypedSelector((store) => store.PostReducer);
  const { loading } = store;
  const [isRedirect, SetIsRedirect] = useState(false);
  const [image, SetImage] = useState("/static/assets/posts/noImage.jpg");
  const { GetAllCategories, AddNewPosts, UpdatePost } = useActions();
  ...
  const changePost = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const ShortDescription: any = data.get("ShortDescription");
    const Description: any = data.get("Description");
    const CategoryId: any = data.get("CategoryId");
